...
class PackageRegistry:
    ...
    def search(
        ...
        """
        self.logger.log(
            f"Searching packages: query='{query}', tag={tag}, category={category}, source={source}",
            "debug",
        )
        results = []
    ...
    def get_stats(self) -> dict[str, int]:
        ...
        return {
            ...
            "sources": len({p.source for p in self._packages.values()}),
            "categories": len({p.category for p in self._packages.values() if p.category}),
        }
