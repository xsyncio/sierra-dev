...
class RepositoryManager:
    ...
    def load_sources(self) -> None:
        ...
        try:
            with open(self.sources_file) as f:
                data = json.load(f)
                self.sources = [RepositorySource.from_dict(s) for s in data.get("sources", [])]
    ...
    def add_source(
        self, url: str, name: str | None = None, branch: str = "main", priority: int = 10
    ) -> RepositorySource:
        """
        ...
        source = RepositorySource(name=name, url=url, branch=branch, priority=priority)
    ...
    def update_registry(self, source_name: str | None = None) -> dict[str, int]:
        ...
        sources_to_update = [self.get_source(source_name)] if source_name else self.list_sources()
        self.logger.log(f"Updating {len(sources_to_update)} source(s)", "debug")

        from concurrent.futures import ThreadPoolExecutor

        def fetch_and_cache(source) -> tuple[str, int]:
            ...
            try:
                ...
                return source.name, pkg_count
            except Exception as e:
                self.logger.log(f"Failed to update {source.name}: {e}", "error")
                return source.name, 0

        valid_sources = [s for s in sources_to_update if s is not None and s.enabled]
        if not valid_sources:
            return results

        with ThreadPoolExecutor(max_workers=min(len(valid_sources), 10)) as executor:
            futures = [executor.submit(fetch_and_cache, source) for source in valid_sources]
            for fut in futures:
    ...
    def _fetch_registry(self, source: RepositorySource) -> dict[str, typing.Any]:
        ...
        raw_url = f"https://raw.githubusercontent.com/{owner}/{repo}/{source.branch}/registry.json"
