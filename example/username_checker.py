...
invoker = sierra.InvokerScript(
    name="username_checker", description="Check username availability across multiple platforms"
)
...
def check_platform(platform: str, username: str) -> tuple[str, str, str]:
    ...
    platforms_config = {
        "github": {"url": f"https://github.com/{username}", "exists_code": 200},
        "twitter": {"url": f"https://twitter.com/{username}", "exists_code": 200},
        "instagram": {"url": f"https://www.instagram.com/{username}/", "exists_code": 200},
        "reddit": {"url": f"https://www.reddit.com/user/{username}", "exists_code": 200},
        "linkedin": {"url": f"https://www.linkedin.com/in/{username}", "exists_code": 200},
        "youtube": {"url": f"https://www.youtube.com/@{username}", "exists_code": 200},
        "medium": {"url": f"https://medium.com/@{username}", "exists_code": 200},
        "pinterest": {"url": f"https://www.pinterest.com/{username}/", "exists_code": 200},
        "tumblr": {"url": f"https://{username}.tumblr.com", "exists_code": 200},
        "twitch": {"url": f"https://www.twitch.tv/{username}", "exists_code": 200},
    }
    ...
    try:
        response = requests.get(
            config["url"], timeout=10, allow_redirects=True, headers={"User-Agent": "Mozilla/5.0"}
        )
...
def run(
    username: sierra.Param[
        str | None,
        sierra.SierraOption(
            description="Username to check across platforms", mandatory="MANDATORY"
        ),
    ],
    platforms: sierra.Param[
        str | None,
        sierra.SierraOption(
            description="Comma-separated platforms to check (default all)", mandatory="OPTIONAL"
        ),
    ] = None,
) -> None:
    """
    ...
    if rows:
        result = sierra.Table(headers=["Platform", "Status", "URL"], rows=rows)
    else:
    ...
    # Summary
    sum(1 for row in rows if "Found" in row[1])
