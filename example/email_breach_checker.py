...
def run(
    email: sierra.Param[
        str | None, sierra.SierraOption(description="Email address to check", mandatory="MANDATORY")
    ],
    check_password: sierra.Param[
        str | None,
        sierra.SierraOption(
            description="Also check if password hash is breached", mandatory="OPTIONAL"
        ),
    ] = None,
) -> None:
    """
    ...
    if hibp_data.get("breached"):
        ...
        if rows:
            result = sierra.Table(
                headers=["Breach", "Date", "Exposed Data", "Affected Accounts"], rows=rows
            )
        else:
        ...
        result = sierra.Table(
            headers=["Status"], rows=[["No breaches found for this email address"]]
        )
