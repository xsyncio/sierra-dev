...
invoker = sierra.InvokerScript(
    name="ip_intelligence", description="Comprehensive IP geolocation and network intelligence"
)
...
def run(
    ip: sierra.Param[
        str | None, sierra.SierraOption(description="IP address to analyze", mandatory="MANDATORY")
    ],
) -> None:
    """
    ...
    rows.append(
        ["Country", f"{ip_data.get('country_name', 'N/A')} ({ip_data.get('country_code', 'N/A')})"]
    )
    rows.append(["Postal Code", ip_data.get("postal", "N/A")])
    rows.append(
        ["Coordinates", f"{ip_data.get('latitude', 'N/A')}, {ip_data.get('longitude', 'N/A')}"]
    )
    ...
    result = sierra.Table(headers=["Field", "Value"], rows=rows)
