...
invoker = sierra.InvokerScript(
    name="port_scanner", description="Fast TCP port scanner with service detection"
)
...
def run(
    target: sierra.Param[
        str | None,
        sierra.SierraOption(
            description="Target IP address or hostname to scan", mandatory="MANDATORY"
        ),
    ],
    ports: sierra.Param[
        str | None,
        sierra.SierraOption(
            description="Ports to scan (e.g. 804438080 or common or 1-1000)", mandatory="OPTIONAL"
        ),
    ] = "common",
    threads: sierra.Param[
        int | None,
        sierra.SierraOption(
            description="Number of concurrent threads (default 50)", mandatory="OPTIONAL"
        ),
    ] = 50,
) -> None:
    """
    ...
    if open_ports:
        ...
        result = sierra.Table(headers=["Port", "Status", "Service"], rows=rows)

    else:
        result = sierra.Table(headers=["Status"], rows=[["No open ports found"]])
